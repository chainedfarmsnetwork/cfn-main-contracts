...
contract("Test MasterChef contract", async (accounts) => {
    ...
    it("Don't allow more than 6% of LP's deposit fees", async () => {
        ...
        await truffleAssert.reverts(masterChef.set(0, 100, 601, true), "set: invalid deposit fee basis points");
    });
    ...
    it("Deposit to fee's wallet", async () => {
        ...
        await masterChef.add(100, token.address, 400, true, {from: accounts[0]});
        await token.approve(masterChef.address, 100, {from: accounts[0]});
        await masterChef.deposit(0, 100, {from: accounts[0]});
    ...
    it("Emission rate same as defined right after deployment", async () => {
        ...
        assert.equal(web3.utils.fromWei(await masterChef.tokenPerBlock.call()), "1");
    });
    ...
    it("Emission rate increases if current supply gets belows starting supply", async () => {
        ...
        assert.equal(web3.utils.fromWei(await masterChef.tokenPerBlock.call()), "1.5");
    });
    ...
    it("Emission rate get capped if current supply gets too much belows starting supply", async () => {
        ...
        assert.equal(web3.utils.fromWei(await masterChef.tokenPerBlock.call()), "3");
    });
    ...
    it("Emission rate decreases if current supply gets higher than starting supply", async () => {
        ...
        assert.equal(web3.utils.fromWei(await masterChef.tokenPerBlock.call()), "0.25");
    });
    ...
    it("Emission rate decreases to 0 if current supply gets higher then (or is equal to) maximum supply's soft cap", async () => {
        ...
        assert.equal(web3.utils.fromWei(await masterChef.tokenPerBlock.call()), "0");
